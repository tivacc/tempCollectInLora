...
{
    M0P_WDT->WDTCON_f.WOV = u8LoadValue;
}
/**
 ******************************************************************************
 ** \brief  WDT初始化函数
...
en_result_t Wdt_Init(stc_wdt_config_t* pstcConfig)
{
    ...
    M0P_WDT->WDTCON_f.WINT_EN = pstcConfig->enResetEnable;
...
{
    uint8_t u8Count;
    u8Count = M0P_WDT->WDTCON_f.WCNTL;
    return u8Count;
}
/**
 ******************************************************************************
