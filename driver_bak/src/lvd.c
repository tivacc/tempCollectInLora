...
#define IS_VALID_FILTER(x)      ( (x) <= LvdFilter20ms )
...
en_result_t Lvd_EnableIrq(en_lvd_irq_type_t enType)
{
    ...
    {   switch (enType)
        {
            case LvdIrqHigh:
                M0P_LVD->CR_f.LEVEL = 1u;
                M0P_LVD->CR_f.RISING = 0u;
                M0P_LVD->CR_f.FALLING = 0u;
                break;
            case LvdIrqRise:
                M0P_LVD->CR_f.LEVEL = 0u;
                M0P_LVD->CR_f.RISING = 1u;
                M0P_LVD->CR_f.FALLING = 0u;
                break;
            case LvdIrqFall:
                M0P_LVD->CR_f.LEVEL = 0u;
                M0P_LVD->CR_f.RISING = 0u;
                M0P_LVD->CR_f.FALLING = 1u;
                break;
            default:
                break;
        }
...
void Lvd_Init(stc_lvd_config_t *pstcConfig)
{
    ...
    M0P_LVD->CR_f.LEVEL2 = pstcConfig->enThreshold;
    M0P_LVD->CR_f.SOURCE_SEL = pstcConfig->enInput;
    M0P_LVD->CR_f.SEL_RST = pstcConfig->bLvdReset;
