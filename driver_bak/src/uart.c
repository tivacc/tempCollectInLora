...
uint16_t Uart_SetBaudRate(uint8_t u8Idx,uint32_t u32pclk,stc_uart_baud_config_t* pstcBaud)
{
    ...
    pstcData->pstcInstance->SCON_f.DBAUD = pstcBaud->u8Dbaud;
    switch(pstcBaud->u8Mode)
    {
        ...
        case UartMode1:
        case UartMode3:
            u16tmload = 0x10000-((u32pclk*(pstcBaud->u8Dbaud+1))/(pstcBaud->u32Baud*32));//单双倍波特率，定时器配置
            break;
        default:
            break; 
    }
...
en_result_t Uart_SendData(uint8_t u8Idx, uint32_t u32Data)
{
    ...
    pstcData->pstcInstance->SBUF =u32Data;
    return Ok;
}
/**
 ******************************************************************************
