...
/**
 * \brief   Gpio_InitIOExt 
 * 
 * \param   [in]  u8Port             GPIO port index
 * \param   [in]  u8Pin              enGpioDirIn/enGpioDirOut
 * \param   [in]  enDir              
 * \param   [in]  bPullup            pull up
 * \param   [in]  bPulldown          pull down
 * \param   [in]  bOdr               open drain
 * \param   [in]  bDrive             drive ability 
 *                                   0 = high
 *                                   1 = low
 * 
 * \retval  en_result_t      
...
{
    return getBit((uint32_t)&M0P_GPIO->P0STA + u8Port * 0x40, u8Pin);
}

en_result_t Gpio_ClearIrq(uint8_t u8Port, uint8_t u8Pin)
{
    setBit((uint32_t)&M0P_GPIO->P0ICLR + u8Port * GPIO_GPSZ, u8Pin, 0);
...
en_result_t Gpio_SetSignalSource(en_gpio_target_t enTarget, en_gpio_timex_in_t enType)
{
    ...
    switch (enTarget)
    {
        ...
        case GpioSignalTm6A2:
            M0P_GPIO->CTRL3_f.TM6_A2_SEL = enType;
            break;
        case GpioSignalTm6A1:
            M0P_GPIO->CTRL3_f.TM6_A1_SEL = enType;
            break;
        case GpioSignalTm6A0:
            M0P_GPIO->CTRL3_f.TM6_A0_SEL = enType;
            break;
        case GpioSignalTm6B2:
            M0P_GPIO->CTRL3_f.TM6_B2_SEL = enType;
            break;
        case GpioSignalTm6B1:
            M0P_GPIO->CTRL3_f.TM6_B1_SEL = enType;
            break;
        case GpioSignalTm6B0:
            M0P_GPIO->CTRL3_f.TM6_B0_SEL = enType;
            break;
