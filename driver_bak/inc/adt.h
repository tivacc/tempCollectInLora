 ...
 ** \brief ADT TIMxX端口定义
 *****************************************************************************/
typedef enum en_adt_TIMxX_port
{
    AdtTIMxA    = 0u,
    AdtTIMxB    = 1u,
}en_adt_TIMxX_port_t;
...
{
    AdtTIMxDisValNorm,          ///< 强制输出无效条件0~3中被选择的条件成立时，TIMx端口正常输出
    AdtTIMxDisValHiZ,           ///< 强制输出无效条件0~3中被选择的条件成立时，TIMx端口输出高阻态
    AdtTIMxDisValLow,           ///< 强制输出无效条件0~3中被选择的条件成立时，TIMx端口输出低电平
    AdtTIMxDisValHigh,          ///< 强制输出无效条件0~3中被选择的条件成立时，TIMx端口输出高电平
}en_adt_pconr_disval_t;

/**
 ******************************************************************************
 ** \brief ADT端口控制 - TIMx强制输出无效条件选择
 **
 ** \note
 ******************************************************************************/
typedef enum en_adt_pconr_dissel
{
    AdtTIMxDisSel0,             ///< 选择强制输出无效条件0
    AdtTIMxDisSel1,             ///< 选择强制输出无效条件1
    AdtTIMxDisSel2,             ///< 选择强制输出无效条件2
    AdtTIMxDisSel3,             ///< 选择强制输出无效条件3
}en_adt_pconr_dissel_t;

/**
 ******************************************************************************
 ** \brief ADT端口控制 - TIMx周期值匹配时端口状态设定
 **
 ** \note
 ******************************************************************************/
typedef enum en_adt_pconr_perc
{
    AdtTIMxPeriodLow,           ///< 计数器计数值与周期值相等时，TIMx端口输出保持为低电平
    AdtTIMxPeriodHigh,          ///< 计数器计数值与周期值相等时，TIMx端口输出设定为高电平
    AdtTIMxPeriodKeep,          ///< 计数器计数值与周期值相等时，TIMx端口输出设定为先前状态
    AdtTIMxPeriodInv,           ///< 计数器计数值与周期值相等时，TIMx端口输出设定为反转电平
}en_adt_pconr_perc_t;

/**
 ******************************************************************************
 ** \brief ADT端口控制 - TIMx比较值匹配时端口状态设定
 **
 ** \note
 ******************************************************************************/
typedef enum en_adt_pconr_cmpc
{
    AdtTIMxCompareLow,          ///< 计数器计数值与GCMxR相等时，TIMx端口输出保持为低电平
    AdtTIMxCompareHigh,         ///< 计数器计数值与GCMxR相等时，TIMx端口输出设定为高电平
    AdtTIMxCompareKeep,         ///< 计数器计数值与GCMxR相等时，TIMx端口输出设定为先前状态
    AdtTIMxCompareInv,          ///< 计数器计数值与GCMxR相等时，TIMx端口输出设定为反转电平
}en_adt_pconr_cmpc_t;

/**
 ******************************************************************************
 ** \brief ADT端口控制 - TIMx端口输出
 **
 ** \note
 ******************************************************************************/
typedef enum en_adt_pconr_port_out
{
    AdtTIMxPortOutLow,      ///< TIMx端口输出设定为低电平
    AdtTIMxPortOutHigh,     ///< TIMx端口输出设定为高电平
}en_adt_pconr_port_out_t;

/**
 ******************************************************************************
 ** \brief ADT端口控制 - TIMx端口功能模式选择
 **
 ** \note
 ******************************************************************************/
typedef enum en_adt_pconr_capc
{
    AdtTIMxCompareOutput,       ///< TIMx端口设定为比较输出功能
    AdtTIMxCaptureInput,        ///< TIMx端口设定为捕获输入功能
}en_adt_pconr_capc_t;

/**
 ******************************************************************************
 ** \brief ADT端口控制 - TIMx计数开始停止端口状态选择
 **
 ** \note
 ******************************************************************************/
typedef enum en_adt_pconr_stastps
{
    AdtTIMxStateSelSS,      ///< 计数开始或停止时，TIMx端口输出由STACB、STPCB决定
    AdtTIMxStateSelKeep,    ///< 计数开始或停止时，TIMx端口输出设定为先前状态
}en_adt_pconr_stastps_t;

/**
 ******************************************************************************
 ** \brief ADT死区控制 - TIMx死区分离设定
 **
 ** \note
 ******************************************************************************/
typedef enum en_adt_dconr_sepa
{
    AdtTIMxDtSeperate,      ///< DTUAR和DTDAR分别设定
    AdtTIMxDtEqual,         ///< DTDAR的值和DTUAR的值自动相等
}en_adt_dconr_sepa_t;
...
typedef enum en_adt_hw_trig
{
    ...
    AdtHwTrigTimxARise,             ///< TIMxA端口上采样到上升沿
    AdtHwTrigTimxAFall,             ///< TIMxA端口上采样到下降沿
    AdtHwTrigTimxBRise,             ///< TIMxB端口上采样到上升沿
    AdtHwTrigTimxBFall,             ///< TIMxB端口上采样到下降沿
...
{
    AdtHwCntTimxALowTimxBRise   = 0,    ///< TIMxA端口为低电平时，TIMxB端口上采样到上升沿
    AdtHwCntTimxALowTimxBFall   = 1,    ///< TIMxA端口为低电平时，TIMxB端口上采样到下降沿
    AdtHwCntTimxAHighTimxBRise  = 2,    ///< TIMxA端口为高电平时，TIMxB端口上采样到上升沿
    AdtHwCntTimxAHighTimxBFall  = 3,    ///< TIMxA端口为高电平时，TIMxB端口上采样到下降沿
    AdtHwCntTimxBLowTimxARise   = 4,    ///< TIMxB端口为低电平时，TIMxA端口上采样到上升沿
    AdtHwCntTimxBLowTimxAFall   = 5,    ///< TIMxB端口为低电平时，TIMxA端口上采样到下降沿
    AdtHwCntTimxBHighTimxARise  = 6,    ///< TIMxB端口为高电平时，TIMxA端口上采样到上升沿
    AdtHwCntTimxBHighTimxAFall  = 7,    ///< TIMxB端口为高电平时，TIMxA端口上采样到下降沿
...
typedef enum en_adt_irq_type
{
    ...
    AdtSAMLIrq = 14,    ///< TIMxA/B端口低状态中断
    AdtSAMHIrq = 15,    ///< TIMxA/B端口高状态中断
}en_adt_irq_type_t;
...
typedef struct stc_adt_TIMxX_port_cfg
{
    ...
    boolean_t               bFltEn;     ///< 触发源滤波使能
    en_adt_fconr_nofick_t   enFltClk;   ///< 滤波采样基准时钟
}stc_adt_TIMxX_port_cfg_t;
