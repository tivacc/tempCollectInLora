...
extern void OnReceiveEvent(void);
 ...
 */
static timer_t MacStateCheckTimer;
 ...
 */
static timer_t TxDelayedTimer;

/*!
 * LoRaMac reception windows timers
 */
static timer_t RxWindowTimer1;
static timer_t RxWindowTimer2;
 ...
 */
static timer_t AckTimeoutTimer;
...
static void OnRadioTxDone( void )
{
    ...
    TimerTime_t curTime = hc32L110RtcGetCounter();
    ...
    {
    	timerStart(&RxWindowTimer1, RxWindow1Delay);
        //TimerSetValue( &RxWindowTimer1, RxWindow1Delay );
        //TimerStart( &RxWindowTimer1 );
        if( LoRaMacDeviceClass != CLASS_C )
        {
        	timerStart( &RxWindowTimer2, RxWindow2Delay);
        }
        if( ( LoRaMacDeviceClass == CLASS_C ) || ( NodeAckRequested == true ) )
        {
            getPhy.Attribute = PHY_ACK_TIMEOUT;
            phyParam = RegionGetPhyParam( LoRaMacRegion, &getPhy );
            timerStart( &AckTimeoutTimer, RxWindow2Delay + phyParam.Value);
        }
    }
    else
    {
...
static void PrepareRxDoneAbort( void )
{
    ...
    //timerStart(&MacStateCheckTimer, 10);			//qiming@20180116
...
static void OnMacStateCheckTimerEvent( void )
{
    ...
    else
    {
    	...
    	timerStart( &MacStateCheckTimer, MAC_STATE_CHECK_TIMEOUT );
    }
...
static void OnRxWindow1TimerEvent( void )
{
	...
	RxWindow1Config.WindowTimeout = 1000;
    ...
    //RxWindowSetup( RxWindow1Config.RxContinuous, 1000000/*LoRaMacParams.MaxRxWindow*/ );
	RxWindowSetup( RxWindow1Config.RxContinuous, 10000);
}
...
static LoRaMacStatus_t ScheduleTx( void )
{
    ...
    else
    {
        ...
        LoRaMacState |= LORAMAC_TX_DELAYED;
        timerStart( &TxDelayedTimer, dutyCycleTimeOff );
...
static void CalculateBackOff( uint8_t channel )
{
    ...
    calcBackOff.ElapsedTime = hc32L110RtcElapse(LoRaMacInitializationTime);
    //calcBackOff.ElapsedTime = 0;
...
LoRaMacStatus_t SendFrameOnChannel( uint8_t channel )
{
    ...
    //TimerStart( &MacStateCheckTimer );
    timerStart(&MacStateCheckTimer, 1500 );
...
LoRaMacStatus_t LoRaMacInitialization( LoRaMacPrimitives_t *primitives, LoRaMacCallback_t *callbacks, LoRaMacRegion_t region )
{
    ...
    LoRaMacInitializationTime = hc32L110RtcGetCounter();
