...
static bool VerifyTxFreq( uint32_t freq, uint8_t *band )
{
    ...
    // if( Radio.CheckRfFrequency( freq ) == false )
		if( SX1276CheckRfFrequency ( freq ) == false )
		{
        return false;
    }
...
uint8_t RegionEU868RxParamSetupReq( RxParamSetupReqParams_t* rxParamSetupReq )
{
    ...
    if(SX1276CheckRfFrequency( rxParamSetupReq->Frequency ) == false )
    {
...
bool RegionEU868NextChannel( NextChanParams_t* nextChanParams, uint8_t* channel, TimerTime_t* time, TimerTime_t* aggregatedTimeOff )
{
    ...
    if( nextChanParams->AggrTimeOff <= timerElapsed( nextChanParams->LastAggrTx ) )
    {
    ...
    {
        delayTx++;
        nextTxDelay = nextChanParams->AggrTimeOff - timerElapsed( nextChanParams->LastAggrTx );
    }
