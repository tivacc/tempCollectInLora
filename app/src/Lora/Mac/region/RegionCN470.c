...
bool RegionCN470RxConfig( RxConfigParams_t* rxConfig, int8_t* datarate )
{
    ...
    if( rxConfig->Window == 0 )
    {
        ...
        frequency = ( (uint32_t) 470.3e6 ) + ( rxConfig->Channel % 48 ) * CN470_STEPWIDTH_RX1_CHANNEL;
    }
    ...
    Radio.SetRxConfig( MODEM_LORA, rxConfig->Bandwidth, phyDr, 1, 0, 8, rxConfig->WindowTimeout, false, 0, false, 0, 0, true, false );
	//Radio.SetRxConfig( MODEM_LORA, rxConfig->Bandwidth, 7, 1, 0, 8, rxConfig->WindowTimeout, false, 0, true, 0, 0, false, true);
...
{	
    ...
    Radio.SetTxConfig( MODEM_LORA, 14,      0, 0, phyDr, 1, 8, false, true, 0, 0, false, 3e6 );
    //Radio.SetTxConfig( MODEM_LORA, TX_OUTPUT_POWER, 0, 0, 12,          1, 8, false, true, 0, 0, false, 3e6 );
...
bool RegionCN470NextChannel( NextChanParams_t* nextChanParams, uint8_t* channel, TimerTime_t* time, TimerTime_t* aggregatedTimeOff )
{
    ...
    //if( nbEnabledChannels > 0 )
    {
        ...
        //*channel = enabledChannels[randr( 6, 14 - 1 )];		//471.5mhz--472.9mhz  sx1301
        *channel = 6;
        currentch = *channel;
